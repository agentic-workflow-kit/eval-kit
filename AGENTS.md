...
`eval-kit` is the shared evaluation infrastructure repo for agentic-workflow-kit. It will own
reusable eval mechanics extracted from consumer repos such as `technical-design`; repo-specific
semantics stay with the consuming repo unless a documented product/design decision moves them here.
Treat public package shape, schema names, fixtures, and command contracts as versioned boundaries
once they exist.
...
Add rows as runtime source, fixtures, schemas, or package entrypoints land; keep this map current
and small.

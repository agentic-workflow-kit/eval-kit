...
2. Choose a safe case id and run `eval-kit scaffold-case --case <id>`.
3. Fill the generated `input.md`, `expected-items.json`, and `rubric.md` with consumer-owned content.
4. Run `eval-kit list-cases` to confirm discovery.
5. Run `eval-kit doctor` to validate manifests and artifacts.
6. If a deterministic candidate is available, run `eval-kit run-case --case <id> --candidate <path>`.
